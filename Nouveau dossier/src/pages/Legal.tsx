...
export default function Legal() {
  ...
  return (
    <div id="legal-container" className={`min-h-screen font-sans flex flex-col justify-between transition-colors duration-300 ${
      d ? 'bg-gradient-hero text-white' : 'bg-gradient-hero-light text-slate-900'
    }`}>
      {/* Header section */}
      <header id="legal-header" className={`h-16 px-6 flex items-center justify-between border-b ${
      ...
      <main id="legal-main" className="flex-1 max-w-4xl w-full mx-auto px-6 py-12 md:py-20">
        <motion.div 
        ...
        >
          ...
          <div className="space-y-4 text-center md:text-left">
            ...
            <h1 className="font-display text-4xl font-black tracking-tight">{t('legal.title')}</h1>
            <p className={`text-base ${d ? 'text-zinc-400' : 'text-slate-500'} max-w-2xl leading-relaxed`}>
              {t('legal.subtitle')}
            </p>
          </div>

          {/* Legal Categories accordion or layouts */}
          <div className="space-y-8 pt-4">
            {/* Terms of Service */}
            <div className={`p-8 rounded-[32px] border ${d ? 'bg-zinc-950/40 border-white/5' : 'bg-white border-slate-200'} space-y-4`}>
              <div className="flex items-center gap-3">
                <FileText className="w-5 h-5 text-accent" />
                <h3 className="font-display text-lg font-bold">{t('legal.p1.title')}</h3>
              </div>
              <div className={`text-xs space-y-3.5 leading-relaxed ${d ? 'text-zinc-400' : 'text-slate-600'}`}>
                <p>
            ...
            <div className={`p-8 rounded-[32px] border ${d ? 'bg-zinc-950/40 border-white/5' : 'bg-white border-slate-200'} space-y-4`}>
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-display text-lg font-bold">{t('legal.p2.title')}</h3>
              </div>
              <div className={`text-xs space-y-3.5 leading-relaxed ${d ? 'text-zinc-400' : 'text-slate-600'}`}>
                <p>
            ...
            <div className={`p-8 rounded-[32px] border ${d ? 'bg-zinc-950/40 border-white/5' : 'bg-white border-slate-200'} space-y-4`}>
              <div className="flex items-center gap-3">
                ...
                <h3 className="font-display text-lg font-bold">{t('legal.p3.title')}</h3>
              </div>
              <div className={`text-xs space-y-3.5 leading-relaxed ${d ? 'text-zinc-400' : 'text-slate-600'}`}>
                <p>
